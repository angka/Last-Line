...
function displayStore(data) {
    ...
    for (const cat of categories) {
        const items = data.cosmetics.filter((c) => c.category === cat);
        if (!items.length)
            continue;
        ...
        ws.on('open', () => {
            rl.question('  Choose a username (3-20 chars, letters/numbers/underscore): ', (username) => {
                rl.question('  Email: ', (email) => {
                    rl.question('  Password (min 6 chars): ', (password) => {
...
function connectNewCharacter(playerId, playerName, token) {
    connectWithAuth(playerId, playerName, token, 1);
}
function connectExistingCharacter(playerId, playerName, token) {
    connectWithAuth(playerId, playerName, token, 1);
}
// ─── Continue Session ──────────────────────────────────────────────────────────
function doContinue(savedSession) {
    // Validate saved session by attempting to connect
    const ws = new ws_1.default(savedSession.serverUrl);
    ws.on('open', () => {
...
function characterSelect(playerId, playerName, token) {
    console.log(divider());
