...
exports.createDefaultAchievementStats = createDefaultAchievementStats;
exports.isAchievementUnlocked = isAchievementUnlocked;
exports.checkAchievementTrigger = checkAchievementTrigger;
...
function isAchievementUnlocked(save, achievementId) {
    return save.achievements?.some(a => a.id === achievementId) ?? false;
}
// ─── Check achievement trigger ───────────────────────────────────────────────
function checkAchievementTrigger(def, stats, currentLevel) {
    const t = def.trigger;
    switch (t.type) {
        case 'kill_count':
...
function checkAndUnlockAchievements(save, stats) {
    const newlyUnlocked = [];
    for (const def of Object.values(exports.ACHIEVEMENTS)) {
        if (isAchievementUnlocked(save, def.id))
            continue;
    ...
    const totalPoints = allUnlocked.reduce((sum, a) => {
        const def = exports.ACHIEVEMENTS[a.id];
        return sum + (def?.points ?? 0);
    }, 0);
    return { newlyUnlocked, totalPoints };
}
// ─── Apply achievement reward to save ────────────────────────────────────────
function applyAchievementReward(save, def) {
    if (!def.reward)
        return save;
    let newSave = { ...save };
...
function formatAchievements(save, stats) {
    const unlocked = new Set((save.achievements ?? []).map(a => a.id));
    const totalPoints = unlocked.size > 0
        ? Object.values(exports.ACHIEVEMENTS).filter(d => unlocked.has(d.id)).reduce((s, d) => s + d.points, 0)
        : 0;
    const categories = ['combat', 'dungeon', 'exploration', 'crafting', 'collection', 'social', 'pvp'];
    ...
    for (const cat of categories) {
        const catAchievements = Object.values(exports.ACHIEVEMENTS).filter(a => a.category === cat);
    ...
    lines.push(`  ╠══════════════════════════════════════════════════════════════════════╣`);
    lines.push(`  ║  Unlocked: ${String(unlocked.size).padStart(2)}/${Object.keys(exports.ACHIEVEMENTS).length}                                            ║`);
    lines.push(`  ╚══════════════════════════════════════════════════════════════════════╝`);
...
function formatAchievementUnlock(def) {
...
function deserializeAchievementSets(data) {
    return {
