...
exports.listCraftableItems = listCraftableItems;
...
function craftItem(save, recipeIndex) {
    ...
    for (const mat of recipe.materials) {
        for (let i = 0; i < mat.qty; i++) {
            ...
            if (slot) {
                const result = (0, InventoryManager_1.inventoryRemove)(s, slot.slotId);
                if (result)
                    s = result;
            }
        }
...
function formatCraftingMenu(save) {
...
const nodeState = {}; // nodeId → uses remaining
function getNodeUsesRemaining(nodeId, maxUses) {
...
function gatherFromNode(save, areaId, verb) {
    ...
    for (const drop of gathered) {
        if (drop.itemId === 'gold') {
            ...
            const result = (0, InventoryManager_1.inventoryAdd)(s, drop.itemId, drop.quantity);
            s = result.save;
        }
    }
...
//# sourceMappingURL=CraftingManager.js.map