...
class LeaderboardManager {
    ...
    getLeaderboard(limit = 10) {
        const entries = [];
        for (const [, session] of PresenceManager_1.presenceManager.getAllSessions()) {
    ...
    getPlayerEntry(playerId) {
        const pSession = PresenceManager_1.presenceManager.getSession(playerId);
        if (!pSession)
            return null;
...
function getPvPTier(points) {
    if (points >= 2400)
        return '🏆 Champion (2400+)';
    if (points >= 2100)
        return '⚔️  Diamond (2100+)';
    if (points >= 1800)
        return '🛡️  Platinum (1800+)';
    if (points >= 1500)
        return '⬜ Gold (1500+)';
    if (points >= 1200)
        return '🥉 Silver (1200+)';
