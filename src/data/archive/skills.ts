...
export const PHYSICAL_SKILLS: Record<string, PhysicalSkill> = {
  power_strike: {
    id: 'power_strike',
    name: 'Power Strike',
    level: 1,
...
export const MAGIC_SKILLS: Record<string, MagicSkill> = {
  fireball: {
    id: 'fireball',
    name: 'Fireball',
    level: 1,
...
export const PHYSICAL_SCROLL_DROPS: Record<string, { itemId: string; skillId: string }> = {
...
export const MAGIC_SCROLL_DROPS: Record<string, { itemId: string; skillId: string }> = {
...
export const SUPPORT_SCROLL_DROPS: Record<string, { itemId: string; skillId: string }> = {
...
export function getScrollDropsForTier(tier: number): string[] {
  const physicalScrolls = Object.values(PHYSICAL_SCROLL_DROPS).map(s => s.itemId);
  const magicScrolls = Object.values(MAGIC_SCROLL_DROPS).map(s => s.itemId);
  const supportScrolls = Object.values(SUPPORT_SCROLL_DROPS).map(s => s.itemId);
...
export function getPhysicalSkill(skillId: string): PhysicalSkill | undefined {
  return PHYSICAL_SKILLS[skillId];
}

export function getMagicSkill(skillId: string): MagicSkill | undefined {
  return MAGIC_SKILLS[skillId];
}

export function getSupportSkill(skillId: string): SupportSkill | undefined {
  return SUPPORT_SKILLS[skillId];
}

export function getSkillByItemId(itemId: string): { type: 'physical' | 'magic' | 'support'; skill: PhysicalSkill | MagicSkill | SupportSkill } | undefined {
  for (const [sid, entry] of Object.entries(PHYSICAL_SCROLL_DROPS)) {
    if (entry.itemId === itemId) {
      const skill = PHYSICAL_SKILLS[entry.skillId];
      return skill ? { type: 'physical', skill } : undefined;
    }
  }
  for (const [sid, entry] of Object.entries(MAGIC_SCROLL_DROPS)) {
    if (entry.itemId === itemId) {
      const skill = MAGIC_SKILLS[entry.skillId];
      return skill ? { type: 'magic', skill } : undefined;
    }
  }
  for (const [sid, entry] of Object.entries(SUPPORT_SCROLL_DROPS)) {
    if (entry.itemId === itemId) {
      const skill = SUPPORT_SKILLS[entry.skillId];
