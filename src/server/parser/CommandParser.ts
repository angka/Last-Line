...
export function parseCommand(cmd: string, save: SaveFile, combatState?: any, _sessionId?: string, playerId?: string): ParseResult {
  ...
  switch (verb) {
    ...
    case 'l': {
      ...
      const areaNodes = formatAreaNodes(save.worldState.currentArea);
...
function handleSkill(args: string[], save: SaveFile, combatState: any): ParseResult {
  if (combatState) {
...
function handleCombatSkill(args: string[], save: SaveFile, combatState: any): ParseResult {
...
function handleLearn(args: string[], save: SaveFile): ParseResult {
  const raw = args[0];
  if (!raw) {
  ...
  if (type === 'physical') {
    const existing = s.skills.physical.find(sk => sk.id === (skill as any).id);
    if (existing) return { text: `You already know ${(skill as any).name}.` };
    s = { ...s, skills: { ...s.skills, physical: [...s.skills.physical, { ...(skill as any), killCount: 0 }] } };
  } else if (type === 'magic') {
    const existing = s.skills.magic.find(sk => sk.id === (skill as any).id);
    if (existing) return { text: `You already know ${(skill as any).name}.` };
    s = { ...s, skills: { ...s.skills, magic: [...s.skills.magic, { ...(skill as any), killCount: 0 }] } };
  } else {
    const existing = s.skills.support.find(sk => sk.id === (skill as any).id);
    if (existing) return { text: `You already know ${(skill as any).name}.` };
    s = { ...s, skills: { ...s.skills, support: [...s.skills.support, { ...(skill as any), linkedKills: 0 }] } };
  }
...
function handleCraft(args: string[], save: SaveFile): ParseResult {
  if (args.length === 0) {
...
function handlePendingLoot(save: SaveFile, subargs?: string[]): ParseResult {
...
function handleDungeonChest(save: SaveFile): ParseResult {
  const areaId = save.worldState.currentArea;
  if (!isDungeonArea(areaId)) {
    return { text: 'You are not in a dungeon.' };
  }
  const dungeonInfo = getDungeonForArea(areaId);
  if (!dungeonInfo) return { text: 'Unknown dungeon.' };
